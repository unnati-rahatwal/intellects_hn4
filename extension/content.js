  ...
  window.renderAccessIqPanel = () => {
    try {
      ...
      if (data.issues && data.issues.length > 0) {
        const cIssues = data.issues.map((iss, i) => ({...iss, globalIdx: i})).filter(i => i.severity === 'critical');
        const wIssues = data.issues.map((iss, i) => ({...iss, globalIdx: i})).filter(i => i.severity !== 'critical');
        ...
        sec.innerHTML = treeHTML;
        
        // Attach click handlers
        sec.querySelectorAll('.issue-node').forEach(node => {
            node.onclick = () => {
                try {
                    const target = document.querySelector(node.getAttribute('data-selector'));
                    if (target) {
                        target.scrollIntoView({ behavior: 'smooth', block: 'center' });
